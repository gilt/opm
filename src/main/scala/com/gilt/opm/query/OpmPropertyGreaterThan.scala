 ...
 */
case class OpmPropertyGreaterThan[T <% Ordered[T]](property: String, value: T) extends OpmPropertyQuery {
  override def isMatch(obj: Any): Boolean = obj.asInstanceOf[T] > value
  override def toMongoDBObject(prefix: String = "") = MongoDBObject("%s%s".format(prefix, property) -> MongoDBObject("$gt" -> toMongo(value)))
}
