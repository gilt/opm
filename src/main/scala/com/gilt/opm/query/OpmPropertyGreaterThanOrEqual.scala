 ...
 */
case class OpmPropertyGreaterThanOrEqual[T <% Ordered[T]](property: String, value: T, valueTranslator: Option[(String, Any) => Any] = None) extends OpmPropertyQuery {
  ...
  override def toMongoDBObject(prefix: String = "", matchInverse: Boolean = false) =
    if (matchInverse) MongoDBObject("%s%s".format(prefix, property) -> MongoDBObject("$lt" -> toMongo(value, translate(property))))
    else MongoDBObject("%s%s".format(prefix, property) -> MongoDBObject("$gte" -> toMongo(value, translate(property))))
}
